...
class FloatingWindowManager: ObservableObject {
    ...
    @Published var isVisible: Bool = true // Window starts visible (SwiftUI WindowGroup shows it)

    private var eventMonitors: [Any] = []
    private var displayLink: CVDisplayLink?
    private var notificationObserver: NSObjectProtocol?
    private var isTrackingSetUp: Bool = false

    // Shake Detection State
    private var lastMouseLoc: CGPoint? = nil
    ...
    private func setupTracking() {
        // Guard against duplicate observer registration if window is set multiple times
        guard !isTrackingSetUp else { return }
        ...
        notificationObserver = NotificationCenter.default.addObserver(forName: NSApplication.didResignActiveNotification, object: nil, queue: .main) { [weak self] _ in
            self?.dismiss()
        }
    }
    ...
    func dismiss() {
        DispatchQueue.main.async {
            ...
            // NOTE: Do NOT stop the display link here.
            // The display link drives handleMouseMoved() which contains the shake detection logic.
            // If we stop it, the user can never re-summon the window by shaking.
        }
    }
    ...
    private func handleMouseMoved() {
        ...
        if let previousMouseLoc = lastMouseLoc {
            let dx = mouseLoc.x - previousMouseLoc.x
            let dy = mouseLoc.y - previousMouseLoc.y
        ...
        if lastDirection != 0, let _ = lastMouseLoc, (currentTime - lastReversalTime > 0.4) {
            shakeReversals = 0
            lastDirection = 0
        }
        ...
        if isVisible {
            ...
            if self.isHovered != isInside {
                // Already on main thread (called from updateFrame's DispatchQueue.main.async)
    ...
    private func updateFrame() {
        // Capture time on the CVDisplayLink thread; dt calculation moves to main thread
        // to avoid cross-thread read/write race on lastUpdateTime.
        let currentTime = CACurrentMediaTime()

        DispatchQueue.main.async {
            // Calculate dt on the main thread to avoid cross-thread race on lastUpdateTime
            let dt = min(currentTime - self.lastUpdateTime, 0.05) // cap dt at 50ms
            self.lastUpdateTime = currentTime

            // Always run mouse tracking for shake detection, even when window is not visible
            self.handleMouseMoved()

            // Only do spring physics when the window is visible
            ...
            if self.isHovered {
                ...
                // Don't stop the display link — we still need it for shake detection
                return
            }
