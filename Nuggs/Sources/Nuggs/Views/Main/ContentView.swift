...
struct ContentView: View {
    ...
    @ObservedObject private var floatingManager = FloatingWindowManager.shared
    ...
    var body: some View {
        ...
        .configureWindow { window in
            ...
            // Sync ignoresMouseEvents with the manager's visibility state.
            // The window starts visible (SwiftUI WindowGroup shows it immediately),
            // so it must be interactive from the start.
            window.ignoresMouseEvents = !FloatingWindowManager.shared.isVisible

            // Connect to manager (this also starts the display link)
